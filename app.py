        ...
        for file in files:
            if file.lower().endswith('.pdf'):
                ...
                rel_path = os.path.relpath(
                    path, PAPERS_DIR).replace(os.sep, '/')
...
def build_cache():
    ...
    for rel_path, mtime in current_files.items():
        ...
        try:
            ...
            text = re.sub(
                r'\s+', ' ', " ".join([page.get_text() for page in doc]))
        except Exception as e:
        ...
        if rel_path in cached_papers and cached_papers[rel_path]['mtime'] == mtime:
            ...
            existing_status = cached_papers.get(
                rel_path, {}).get("status", "none")
            existing_uuid = cached_papers.get(
                rel_path, {}).get("uuid", uuid.uuid4().hex)
            title = None

            try:
                if doc:
                    first_page_text = re.sub(
                        r'[^a-zA-Z0-9\s]', ' ', doc[0].get_text("text"))[:150].strip()
                    if first_page_text:
    ...
    for u_rel_path, u_data in new_cache_papers.items():
        ...
        for v_rel_path, v_data in new_cache_papers.items():
            if u_rel_path == v_rel_path:
                continue

            v_title_clean = v_data["title"].lower().strip()
            if len(v_title_clean) < 10:
                continue
            ...
            if title_idx != -1:
                ...
                if brackets:
                    ...
                    if nums:
                        ...
                        years = re.findall(
                            r'\b([12]\d{3}[a-z]?)\b', ref_context)
                        if years:
                            marker = years[-1]

                in_text_sentences = []
                bib_start = max(0, title_idx - 80)
                bib_end = min(len(u_text), title_idx +
                              len(v_data["title"]) + 120)

                safe_bib_entry = html.escape(
                    "... " + u_text[bib_start:bib_end].strip() + " ...")
                safe_v_title = html.escape(v_data['title'][:40])

                try:
                    bib_entry_highlighted = re.sub(
                        f"({re.escape(safe_v_title)}[^\.]*)", r"<span class='highlight-text'>\1</span>", safe_bib_entry, flags=re.IGNORECASE)
                except:
                    bib_entry_highlighted = safe_bib_entry

                in_text_sentences.append(
                    f"<div style='margin-bottom:8px; color:var(--text-muted);'><i class='fas fa-book'></i> <b>Bibliography Match:</b></div>{bib_entry_highlighted}")
                    ...
                    for sent in u_sentences:
                        if marker.lower() in sent.lower():
                            if search_title not in sent.lower():
                                ...
                                try:
                                    sent_highlighted = re.sub(
                                        f"({re.escape(safe_marker)})", r"<span class='highlight-text'>\1</span>", safe_sent, flags=re.IGNORECASE)
                                    in_text_sentences.append(
                                        f"<div style='margin-bottom:8px; color:var(--text-muted); margin-top:16px;'><i class='fas fa-quote-left'></i> <b>In-Text Citation:</b></div> {sent_highlighted}")
                                except:
                                    in_text_sentences.append(
                                        f"<div style='margin-bottom:8px; color:var(--text-muted); margin-top:16px;'><i class='fas fa-quote-left'></i> <b>In-Text Citation:</b></div> {safe_sent}")
...
def node_details(paper_uuid):
    ...
    for f in os.listdir(NOTES_DIR):
        if f.startswith(prefix):
            ...
            attachments.append({"filename": html.escape(
                f), "original_name": html.escape(original_name)})
...
def upload_attachment():
    ...
    if file.filename == '' or not paper_uuid:
...
def build_graph():
    ...
    elements = [{"group": "nodes", "data": {"id": p["id"], "uuid": p["uuid"],
                                            "label": p["title"], "status": p["status"]}} for p in papers]
